...
echo "Delete the existing files in Execution directory"
rm -rf ${bamboo.REMOTE_DIR}/*
echo "Copy files from Home to Execution directory"
cp -R ${bamboo.HOME_DIR}/pipeline/docker-consul/* ${bamboo.REMOTE_DIR}

cd ${bamboo.REMOTE_DIR}
source ./env/setEnv.sh

export COMPOSE_COMMAND="agent -server -node=${bamboo.HOSTNAME} -bootstrap-expect=${CONSUL_BOOTSTRAP_NO} -ui -advertise=${CONSUL_ADVERTISE_ADDR} -retry-join=10.6.232.89:8381 -retry-join=10.6.232.90:8381 -retry-join=10.6.232.91:8381 -client=0.0.0.0 -serf-lan-port=8381 -serf-wan-port=8382 -server-port=8380"
...
# echo ${bamboo.AD_PASSWORD} | sudo -u moning292 -S -E docker-compose -f docker/docker-compose-consul-server.yml pull
